...
#include <stdio.h>
#include "version.h"
#include "rb2_include.h"
#include "rb2e_include.h"
...
void SetVenueHook(int *thisMetaPerformer, Symbol venue)
{
    ...
    RB2E_MSG("Setting venue: %s", venue.sym);

    RB2E_SendEvent(RB2E_EVENT_VENUE_NAME, venue.sym, strlen(venue.sym));
...
void *NewFileHook(char *fileName, int flags)
{
    ...
    new_path = RB2E_GetRawfilePath(fileName, 0);
    if (new_path != NULL)
    {
        fileName = new_path;
...
LOAD_ORIGINAL:
    if (config.LogFileAccess)
        RB2E_MSG("File: %s (%s)", fileName, (flags & 0x10000) ? "Raw" : "ARK");
...
DataArray *DataReadFileHook(char *path, int dtb)
{
    ...
    {
        rawpath = RB2E_GetRawfilePath(path, 0);
        if (rawpath != NULL)
        {
            RB2E_DEBUG("DataReadFile: %s", rawpath);
...
void *ModifierManagerConstructorHook(int thisModifierManager, int unk)
{
    ...
    RB2E_MSG("Adding custom modifiers...", NULL);
...
// a TextStream object that prints straight to RB2E_DEBUG
...
{
    RB2E_EventStagekit event;
    event.LeftChannel = state1;
    event.RightChannel = state2;
    RB2E_SendEvent(RB2E_EVENT_STAGEKIT, &event, sizeof(RB2E_EventStagekit));
...
void ApplyConfigurablePatches()
{
    ...
    if (config.RenderResX != 0 && config.RenderResY != 0)
    {
        ...
        {
            RB2E_MSG("Render resolution set above 1280x720! This will cause graphical issues.", NULL);
        }
...
void InitialiseFunctions()
{
    ...
    RB2E_MSG("Functions initialized!", NULL);
}
