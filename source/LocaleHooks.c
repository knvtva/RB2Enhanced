...
#include "rb2/Locale.h"
#include "rb2enhanced.h"

#ifdef RB2EDEBUG
static char *rb2e_version_motd = "RB2Enhanced D-" RB2E_BUILDTAG " is active. Thank you for playing!";
static char *rb2e_mod_motd_fmt = "RB2Enhanced D-" RB2E_BUILDTAG " (with %s) is active. Thank you for playing!";
#else
static char *rb2e_version_motd = "RB2Enhanced " RB2E_BUILDTAG " is active. Thank you for playing!";
static char *rb2e_mod_motd_fmt = "RB2Enhanced " RB2E_BUILDTAG " (with %s) is active. Thank you for playing!";
#endif

// Localisation messages to be replaced (that can be replaced in DTA with rb2e_ at the start of the regular name)
// Left: the translation ID as in locale_keep.dta, Right: the text to replace with
static char *overrideLocales[][2] = {
#ifdef RB2E_XBOX // xbox-specific locale changes (re liveless)
...
char RB2E_ActiveLocale[RB2E_LANG_LEN + 1] = {0};
...
void SetSystemLanguageHook(Symbol lang, int r4)
{
    ...
    strncpy(RB2E_ActiveLocale, language.sym, RB2E_LANG_LEN);
    RB2E_DEBUG("Setting language '%s'", RB2E_ActiveLocale);
...
char *LocalizeHook(int thisLocale, Symbol sym, int fail)
{
    ...
    {
        // check for "rb2e_mod_string"
        SymbolConstruct(&newLocale, "rb2e_mod_string");
        original = Localize(thisLocale, newLocale, fail);
        if (original == NULL)
            return rb2e_version_motd;
        // make sure the new string won't buffer overflow
        if (strlen(original) >= 0x20)
            return rb2e_version_motd;
        sprintf(mod_locale_string, rb2e_mod_motd_fmt, original);
    ...
    for (i = 0; i < numOverrideLocales; i++)
    {
        if (strcmp(sym.sym, overrideLocales[i][0]) == 0)
        {
            ...
            // check if rb2e_localename exists
            sprintf(newLocaleName, "rb2e_%s", sym.sym);
