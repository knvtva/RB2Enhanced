...
static int STUN_ParseIPFromResponse(char *response, int response_length)
{
    ...
    {
        RB2E_DEBUG("Response was not of BINDING-RESPONSE!");
        return -1;
    }
    if (header->size > response_length - sizeof(STUN_Header))
    {
        RB2E_DEBUG("Mismatched STUN header size! Header: %i, packet: %i", header->size, response_length);
        return -1;
    }
    ...
    while (ipv4 == NULL)
    {
        ...
        case 0x0001: // MAPPED-ADDRESS
            ...
            RB2E_DEBUG("Got MAPPED-ADDRESS attribute!", NULL);
            break;
        case 0x0020: // XOR-MAPPED-ADDRESS
            ipv4 = (STUN_AttrIPv4 *)((char *)attribute + sizeof(STUN_AttrHeader));
            RB2E_DEBUG("Got XOR-MAPPED-ADDRESS attribute!", NULL);
            ipv4->ip ^= header->cookie;
            break;
        default:
            RB2E_DEBUG("Unknown attribute %04x, length %04x", attribute->type, attribute->length);
            break;
        }
    ...
    if (ipv4 == NULL)
    {
        RB2E_DEBUG("Failed to get IP address");
...
int STUN_GetExternalIP(char *stunServerIP, short stunPort, char *returnedIP, int socket_type)
{
    ...
    RB2E_DEBUG("Attempting to get external IP address via STUN...", NULL);
    ...
    RB2E_DEBUG("Creating STUN socket...", NULL);
    socket = RB2E_CreateSocket(socket_type);
    if (socket == INVALID_SOCKET)
        return -1;
    RB2E_SetRecvTimeout(socket, 4000);

    RB2E_DEBUG("Sending STUN request...", NULL);
    RB2E_UDP_SendTo(socket, inetAddr.address, stunPort, stunRequest, sizeof(stunRequest));

    RB2E_DEBUG("Receiving STUN response...", NULL);
    r = RB2E_UDP_RecvFrom(socket, NULL, NULL, recvBuffer, sizeof(recvBuffer));
    if (r == SOCKET_ERROR)
        return -1;

    RB2E_DEBUG("Parsing STUN response...", NULL);
    ...
    RB2E_DEBUG("Closing STUN socket...", NULL);
    RB2E_DisposeSocket(socket);
