...
void UPNP_Poll()
{
    ...
    if (!has_tried_init)
    {
        ...
        if (start_ret != 0)
        {
            RB2E_DEBUG("Couldn't start up UPnP, error %i", start_ret);
        }
        has_tried_init = 1;
        return;
    }
    ...
    {
        if (ticker == 0)
            RB2E_DEBUG("UPnP is doing work...", NULL);
    ...
    else if (current_upnp_task == UPnP_SearchingDevicesWANIP)
    {
        ...
        if (ret == 0)
        {
            ...
            {
                RB2E_DEBUG("Found a WANIPConnection device at IP %08x, relative path %s, UUID %s", device.address.sin_addr.S_un.S_addr, device.relative_path, device.uuid);
            ...
            {
                RB2E_DEBUG("Finding WANIPConnection device timed out after 3 seconds, trying WANPPPConnection", NULL);
                NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
        ...
        {
            RB2E_DEBUG("UpnpSearchGetDevices returned %i, failing", ret);
            current_upnp_task = UPnP_Fail;
        }
    }
    else if (current_upnp_task == UPnP_SearchingDevicesWANPPP)
    {
        ...
        if (ret == 0)
        {
            if (devices_returned >= 1)
            {
                RB2E_DEBUG("Found a WANPPPConnection device at IP %08x, relative path %s, UUID %s", device.address.sin_addr.S_un.S_addr, device.relative_path, device.uuid);
                NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
                NetDll_UpnpDescribeCreate(NETDLL_TITLE, &device, 24, 24, NULL, &doing_work);
                current_upnp_task = UPnP_SearchingDescriptions;
                seconds_passed = 0;
            ...
            {
                RB2E_DEBUG("Finding WANPPPConnection device timed out after 3 seconds, failing", NULL);
                NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
                current_upnp_task = UPnP_Fail;
            }
        }
    ...
    else if (current_upnp_task == UPnP_SearchingDescriptions)
    {
        ...
        if (ret == 1)
        {
            if (seconds_passed > 4)
            {
                RB2E_DEBUG("Getting device descriptions from device timed out after 5 seconds, failing", NULL);
                NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
                current_upnp_task = UPnP_Fail;
            }
        }
        else
        {
            ...
            RB2E_DEBUG("Got %i devices %i services with ret %i", num_device_descriptions, num_service_descriptions, ret);
            ...
            for (i = 0; i < num_service_descriptions; i++)
            {
                ...
                {
                    RB2E_DEBUG("Found service type %s with control URL %s", desc->type, desc->control_url);
                    found_service = 1;
            ...
            if (!found_service)
            {
                RB2E_DEBUG("No WAN service found, failing", NULL);
                current_upnp_task = UPnP_Fail;
            }
            else // request our external IPv4 address
            {
                ...
                RB2E_DEBUG("Found WAN service, performing GetExternalIPAddress", NULL);
    ...
    else if (current_upnp_task == UPnP_ObtainingExternalIP)
    {
        ...
        if (ret == 1)
        {
            if (seconds_passed > 2)
            {
                RB2E_DEBUG("Getting external IPv4 from device timed out after 3 seconds, failing", NULL);
                NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
                current_upnp_task = UPnP_Fail;
            }
        }
        else
        {
            int i = 0;
            char got_ip = 0;
            RB2E_DEBUG("Got response from external IPv4 request with ret %i", ret);
            for (i = 0; i < num_out_params; i++)
            {
                if (out_params[i].name != NULL && strcmp(out_params[i].name, "NewExternalIPAddress") == 0)
                {
                    RB2E_DEBUG("UPnP gateway IP: %s", out_params[i].values->string);
                    strncpy(config.ExternalIP, out_params[i].values->string, RB2E_MAX_IP_LEN);
                    got_ip = 1;
                }
            }
            NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
            if (!got_ip)
            {
                RB2E_DEBUG("No IPv4 address found, failing", NULL);
                current_upnp_task = UPnP_Fail;
            }
            else
            {
                unsigned int my_ipv4 = RB2E_GetInternalIP();
    ...
    else if (current_upnp_task == UPnP_ObtainingPort)
    {
        ...
        if (ret == 1)
        {
            if (seconds_passed > 2)
            {
                RB2E_DEBUG("Mapping port from device timed out after 3 seconds, failing", NULL);
                NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
                current_upnp_task = UPnP_Fail;
            }
        }
        ...
        {
            NetDll_UpnpCloseHandle(NETDLL_TITLE, doing_work);
            RB2E_DEBUG("Succesfully mapped port 9103 via UPnP!", NULL);
