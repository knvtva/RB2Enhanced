...
{
    RB2E_MSG("Loading directory %s", thisDirLoader->mFilePath.buf);
...
void LoadObj(Object *object, BinStream *stream)
{
    ...
    replacementPath = RB2E_GetRawfilePath(newPath, 0);
    if (replacementPath != NULL)
    {
        RB2E_DEBUG("Replacing Milo scene asset %s with file at %s", object->name, replacementPath);
        ...
        RB2E_DEBUG("Endianness of replacement file is: %s", (endianResult > 100) ? "Little endian" : "Big endian");
...
// This hook allows for GH2-360/RB1/RB2 meshes to load correctly in RB2
void VertexReadHook(BinStream *thisBinStream, Vector3 *vec3)
{
    ...
    // RB2 (for whatever reason) expects vertices in these versions of meshes to not include W
