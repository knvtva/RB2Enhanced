/*
    RB2Enhanced - net_liveless_online.c
...
#include "rb2/PassiveMessagesPanel.h"
#include "rb2/RockCentralGateway.h"
#include "xbox360.h"
#include "version.h"

// done in LocaleHooks.c
extern char RB2E_ActiveLocale[RB2E_LANG_LEN + 1];
...
void Liveless_DoClientHello()
{
    ...
    hello->using_xenia = RB2E_IsEmulator();
    strncpy(hello->language, RB2E_ActiveLocale, 3);
    strncpy(hello->rb2e_version, RB2E_BUILDTAG, sizeof(hello->rb2e_version));

    RB2E_TCP_Send(Liveless_Socket, &packet, sizeof(LLMM_PacketHeader) + sizeof(LLMM_ClientHello));
}
...
void Liveless_DoPong()
{
    ...
    RB2E_TCP_Send(Liveless_Socket, &packet, sizeof(LLMM_PacketHeader));
}
...
void Liveless_DoJoinUser(char *user_code)
{
    ...
    RB2E_TCP_Send(Liveless_Socket, &packet, sizeof(LLMM_PacketHeader) + sizeof(LLMM_UserJoinRequest));
}
...
void Liveless_DoClientLogin(int player_num, char make_proof, unsigned char *proof_key)
{
    ...
    RB2E_TCP_Send(Liveless_Socket, &packet, sizeof(LLMM_PacketHeader) + sizeof(LLMM_ClientLogin));
}
...
void Liveless_Disconnect()
{
    ...
    RB2E_DisposeSocket(Liveless_Socket);
}
...
void Liveless_StartConnect(void *arg)
{
    ...
    RB2E_DEBUG("Resolving %s for Liveless...", config.LivelessAddress);
    ...
    {
        RB2E_DEBUG("Failed to resolve Liveless domain", NULL);
        connection_error = 1;
        connecting = 0;
        return;
    }
    else
        RB2E_DEBUG("Resolved Liveless server domain to %08x", liveless_address.address);

    // create the socket
    Liveless_Socket = RB2E_CreateSocket(RB2E_TYPE_TCP);
    // set the socket to timeout after 4 seconds
    RB2E_SetTimeout(Liveless_Socket, 4000);

    // try connect to the server
    r = RB2E_TCP_Connect(Liveless_Socket, liveless_address.address, LLMM_PORT);
    if (r < 0)
    {
        RB2E_DEBUG("Connection failed %i %i", r, WSAGetLastError());
        connection_error = 2;
        connecting = 0;
        return;
    }
    // set non-blocking
    RB2E_SetNonBlocking(Liveless_Socket, 1);
...
void Liveless_DelayedAction(void *arg)
{
    ...
    if (delayed_task == 0)
    {
        XShowKeyboardUI(last_player_num, VKBD_LATIN_GAMERTAG, NULL, L"RB2Enhanced Liveless", L"Enter the user code of the user you would like to join. (Case insensitive)", entered_code, 9, &overlapped);
    ...
    else if (delayed_task == 1)
    {
        ...
        XShowMessageBoxUI(last_player_num, L"RB2Enhanced Liveless", dialog_text_buffer, 1, buttons, 0, XMB_ALERTICON, &result, &overlapped);
...
void Liveless_ShowConnectedUI(player_num)
{
    ...
    XShowMessageBoxUI(player_num, L"RB2Enhanced Liveless", dialog_text_buffer, 3, buttons, 2, XMB_ALERTICON, &result, &overlapped);
...
void Liveless_Poll()
{
    ...
    if (XHasOverlappedIoCompleted(&overlapped) && last_dialog_shown != Dialog_JustAnOkay)
    {
        ...
        if (last_dialog_shown == Dialog_NotConnected)
        {
            ...
            { // 'Connect to Server' button
                ...
                RB2E_CreateThread(Liveless_StartConnect, NULL, 0x1000);
            }
        }
        else if (last_dialog_shown == Dialog_Connected)
        {
            ...
            { // 'Join User Code' button
                RB2E_CreateThread(Liveless_DelayedAction, NULL, 0x1000);
                delayed_task = 0;
            }
        ...
        else if (last_dialog_shown == Dialog_EnterUserCode)
        {
            ...
            {
                RB2E_DEBUG("Joining user code %.8S", entered_code);
    ...
    {
        RB2E_DEBUG("Connection with Liveless server timed out, disconnecting.", NULL);
        connection_error = 4;
        Liveless_Disconnect();
        return;
    }
    ...
    r = RB2E_TCP_Recv(Liveless_Socket, &recv_buf, sizeof(LLMM_Packet));
    if (r < 0) // packet error
    {
        int err = RB2E_LastError();
        ...
        if (err != 10035 && err != 0 && (!RB2E_IsEmulator() || (err != 10038 && err > 10000)))
        {
            // connection error
            RB2E_DEBUG("Connection to Liveless socket severed, ret %i err %i", r, err);
            connection_error = 3;
            Liveless_Disconnect();
            return;
        }
    ...
    else if (r > 0) // got a packet, do something about it
    {
        if (r < sizeof(LLMM_PacketHeader))
            RB2E_DEBUG("Recieved invalid packet size %i from Liveless server.", r);
        else if (recv_buf.header.packet_magic != LLMM_MAGIC)
            RB2E_DEBUG("Recieved invalid packet magic %04x from Liveless server.", recv_buf.header.packet_magic);
        else
        {
            frames_since_last_packet = 0;
            ...
            else if (recv_buf.header.packet_type == LL_ServerLoggedIn)
            {
                ...
                RB2E_DEBUG("Liveless server logged in - IP: %08x, user code %.8s", packet->public_ipv4, packet->user_code);
            ...
            else if (recv_buf.header.packet_type == LL_JoinResponse)
            {
                ...
                RB2E_DEBUG("Got a join response from the Liveless server, public/private %08x %08x, username %.16s", packet->public_ipv4, packet->private_ipv4, packet->joining_user);
            ...
            else if (recv_buf.header.packet_type == LL_NATPunchRequest)
            {
                ...
                RB2E_DEBUG("Server is asking us to punch through to public IP %08x", packet->public_ipv4);
                if (game_socket != NULL)
                {
                    RB2E_DEBUG("We've got a game socket so send it", NULL);
                    RB2E_UDP_SendTo((int)game_socket->native_socket, packet->public_ipv4, 9103, "NAT!", 4);
                }
            }
            else
            {
                RB2E_DEBUG("Got unknown packet type %i from the Liveless server", recv_buf.header.packet_type);
            }
        }
    }
    else
...
int Liveless_ShowUI(int player_num)
{
    ...
    if (Liveless_Connected == 0 && connecting == 0)
    {
        ...
        XShowMessageBoxUI(player_num, L"RB2Enhanced Liveless", L"You are not connected to the Liveless server. If you are using direct IP connections, go to 'Play Now' -> 'Find Online Players'.", 2, buttons, 0, XMB_ALERTICON, &result, &overlapped);
    }
    ...
    {
        RB2E_MSG("WTF liveless state, blame Emma", NULL);
    }
    return 0;
}
