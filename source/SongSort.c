...
#define RB2E_SONGSORT_START 10
#define NEW_ORIGIN_COUNT 512
static const char *defaultOrigins[9] = {"rb1", "rb2", "rb2", "greenday", "pearljam", "lego", "rb1_dlc", "rb2_dlc", "ugc_plus"};
...
{
    if (gameOrigin >= RB2E_SONGSORT_START)
    {
        // the original func is updating the symbol provided in r3 so do the same here
        sym->sym = origins[gameOrigin - RB2E_SONGSORT_START].sym;
        return sym;
    }
...
int GetGameOriginBySymbolHook(Symbol gameOrigin)
{
    ...
    for (i = 0; i < newOrigins; i++)
    {
        ...
        if (strcmp(origins[i].sym, gameOrigin.sym) == 0)
            return i + RB2E_SONGSORT_START;
    }
    if (newOrigins >= NEW_ORIGIN_COUNT)
    {
        RB2E_MSG("Over the origin limit trying to add %s..!?", gameOrigin.sym);
        return 0;
    }
    // we haven't seen this origin before, construct the new origin
    r = newOrigins + RB2E_SONGSORT_START;
