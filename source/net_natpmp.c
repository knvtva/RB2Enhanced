...
void NATPMP_RequestExternalIP()
{
    ...
    RB2E_UDP_SendTo(natpmp_socket, gateway_ipv4, PCP_COMMAND_PORT, &req, sizeof(PCP_PacketHeader));
    RB2E_DEBUG("Requested external IP from NATPMP", NULL);
}
...
void NATPMP_RequestOpenPort(unsigned short port)
{
    ...
    // fire it off to the gateway
    RB2E_UDP_SendTo(natpmp_socket, gateway_ipv4, PCP_COMMAND_PORT, &req, sizeof(NATPMP_MappingRequest));
    RB2E_DEBUG("Requested port %i from NATPMP", port);
}
...
void PCP_RequestOpenPort(unsigned short port)
{
    ...
    fill_ipv4_mapped_ipv6(&req.header.local_ipv6, RB2E_GetInternalIP());
    ...
    // fire it off to the gateway
    RB2E_UDP_SendTo(natpmp_socket, gateway_ipv4, PCP_COMMAND_PORT, &req, sizeof(NATPMP_MappingRequest));
    RB2E_DEBUG("Requested port %i from PCP", port);
}
...
void NATPMP_Init()
{
    ...
    natpmp_socket = RB2E_CreateSocket(RB2E_TYPE_UDP);
    // do we need to bind? don't think so
    // RB2E_BindPort(natpmp_socket, PCP_LISTEN_PORT);
    RB2E_SetNonBlocking(natpmp_socket, 1);
    //  get the address of the gateway
    gateway_ipv4 = RB2E_GetGatewayIP();
    // set a random nonce to use for pcp mappings
    for (i = 0; i < sizeof(mapping_nonce); i++)
        mapping_nonce[i] = RandomInt(0, 255);
    RB2E_DEBUG("Set up NATPMP socket for gateway %08x", gateway_ipv4);
}
...
void NATPMP_Poll()
{
    ...
    {
        RB2E_DEBUG("NATPMP request didn't respond within 5 seconds :/", NULL);
        RB2E_DisposeSocket(natpmp_socket);
        natpmp_socket = -1;
    ...
    if (natpmp_socket != -1)
    {
        ...
        int r = RB2E_UDP_RecvFrom(natpmp_socket, &recv_ip, &recv_port, recv_buffer, sizeof(recv_buffer));
        if (r >= 0)
        {
            PCP_PacketHeader *header = (PCP_PacketHeader *)recv_buffer;
            if (recv_ip != gateway_ipv4)
            {
                RB2E_DEBUG("Recieved packet from %08x, but that isn't gateway %08x, ignoring.", recv_ip, gateway_ipv4);
                return;
            }
            sending_init_packet = 0;
            ...
            if (header->response_code == 1)
            {
                if (failed_pcp == 0)
                {
                    RB2E_DEBUG("Gateway told us it doesn't understand PCP, fallback to NAT-PMP.", NULL);
                    failed_pcp = 1;
                ...
                {
                    failed_natpmp = 1;
                    RB2E_DEBUG("Gateway told us it doesn't understand NATPMP, give up.", NULL);
                    RB2E_DisposeSocket(natpmp_socket);
                    natpmp_socket = -1;
                }
                return;
            ...
            if (header->version == NATPMP_VERSION)
            {
                if (header->opcode == (0x80 | NATPMP_OP_GET_IP) && header->response_code == 0)
                {
                    ...
                    RB2E_DEBUG("Gateway gave us external IPv4 %08x", resp->external_ipv4);
                ...
                else if (header->opcode == (0x80 | NATPMP_OP_UDP_MAP) && header->response_code == 0)
                {
                    ...
                    RB2E_DEBUG("Gateway assigned us port %i for internal %i, for %i seconds", resp->external_port, resp->internal_port, resp->mapping_lifetime);
                ...
                {
                    RB2E_DEBUG("Gateway responded with unknown NATPMP opcode %02X, response code %i", header->opcode, header->response_code);
                }
            }
            ...
            else if (header->version == PCP_VERSION)
            {
                if (header->opcode == (0x80 | PCP_OP_MAP) && header->response_code == 0)
                {
                    ...
                    RB2E_DEBUG("Gateway assigned us external %08x port %i for internal %i, for %i seconds", resp->assigned_address.ipv4, resp->external_port, resp->internal_port, resp->header.lifetime);
                ...
                {
                    RB2E_DEBUG("Gateway responded with unknown PCP opcode %02X, response code %i", header->opcode, header->response_code);
                }
            }
            //
            else
            {
                RB2E_DEBUG("Gateway replied with unknown version %02x, opcode %02x, response code %i", header->version, header->opcode, header->response_code);
            }
        }
        else if (r < 0 && !AWAIT_SOCKET(r))
        {
            RB2E_DEBUG("NATPMP socket error %i %i", r, RB2E_LastError());
            RB2E_DisposeSocket(natpmp_socket);
            natpmp_socket = -1;
        }
        if (time_until_expiry == 0)
        {
            RB2E_DEBUG("Expiry timer exceeded, renewing port mapping...", NULL);
            time_until_expiry = -1;
            if (failed_pcp)
                NATPMP_RequestOpenPort(9103);
            else
