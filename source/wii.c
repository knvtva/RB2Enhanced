...
char ACURL[RB2E_MAX_DOMAIN + 16];
char PRURL[RB2E_MAX_DOMAIN + 16];
...
static int DetectDolphin()
{
    ...
    {
        // otherwise we aren't
        // TODO: Ye Olde Dolphin will report not emulator. Those versions probably wouldn't run RB2E at all tbh.
        return 0;
    }
}
...
int RB2E_RelaunchGame()
{
    /*
        ...
        int file = RB2E_OpenFile("rb2/rb2e/launcher.dol", 0);
        int filesize = RB2E_FileSize(file);
        RB2E_ReadFile(file, 0, buffer, filesize);
        RB2E_CloseFile(file);
...
static void CTHook(void *ThisApp, int argc, char **argv)
{
    ...
    {
        RB2E_DEBUG("Applying WFC patches...", NULL);
        // craft naswii URLs
    ...
    if (config.LegacySDMode && RB2E_Mounted)
    {
        RB2E_DEBUG("Unmounting SD card...", NULL);
        FAT_partition_destructor(&sd_partition);
        __io_wiisd.shutdown();
        RB2E_Mounted = 0;
    }
...
// don't use HookFunction here if it's repeated in rb2enhanced.c
